...
An optional Judgement pass (`judge.mjs`) asks Jev, TypeSafe's System One model, whether each tagged test asserts what its requirement says, and sorts tests into ok, flag and uncertain so `code-review`'s Spec sub-agent reads only the flagged and uncertain ones. It runs only when `TYPESAFE_API_KEY` is set and `docs/agents/traceability.md` has a `Judgement: jev` line, and it never fails a build (ADR 0003). Each line of `docs/agents/traceability.md` is now independently optional.
