...
- **The main flow**, idea to ship. Grill, spec, tickets, implement, review, with two branches inside it: a prototype detour when a question needs runnable code to settle, and the spec-and-tickets split, which only earns its cost when the build spans more than one session. Run [requirement-traceability](https://aihero.dev/skills-requirement-traceability) after the tickets and it adds requirement IDs to the spec and a `Covers:` line to each ticket; a spec carrying IDs then turns on the coverage check through the build.
