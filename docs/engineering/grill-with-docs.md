...
Into the conversation only. This is the most substantive open complaint about the skill: the glossary is not a spec, most answers do not earn an ADR, and nothing ties each resolved answer through to a spec. Precise answers (ordering guarantees, negative requirements, numeric defaults) get softened into weaker prose downstream, and the result can look complete while missing the thing you actually decided. The mitigation available today has two halves. Before the spec, keep the session and feed it straight to [to-spec](https://aihero.dev/skills-to-spec), and re-read the spec against your own answers rather than assuming it captured them. From the spec onward, run [requirement-traceability](https://aihero.dev/skills-requirement-traceability) after `to-tickets`: it puts an ID on each user story, carries it through the tickets and into test names, and reports any requirement no test names.
