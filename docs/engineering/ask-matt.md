...
- **The main flow**, idea to ship. Grill, spec, tickets, implement, review, with two branches inside it: a prototype detour when a question needs runnable code to settle, and the spec-and-tickets split, which only earns its cost when the build spans more than one session. Ask [to-spec](https://aihero.dev/skills-to-spec) for traceable requirements and a spec carrying requirement IDs turns on [requirement-traceability](https://aihero.dev/skills-requirement-traceability) through the build and the review.
