...
Run `scripts/test-traceability-check.sh` after touching the coverage gap check (`check.mjs`) or the `Covers:` structural check in [requirement-traceability](./skills/engineering/requirement-traceability/SKILL.md). It runs `check.mjs` and the structural check's bash block (extracted from the `SKILL.md`, so it is tested as the skill writes it) against `scripts/fixtures/traceability`. Run `node --test scripts/test-tags.mjs` after touching `tags.mjs`, the rules `check.mjs` and `judge.mjs` share (where a tag counts, skips, where a test ends); it runs against `scripts/fixtures/judge`. Behaviour checks on what the model does with a skill live in [evals/](./evals/README.md), run with `claude plugin eval . --scaffold --allow-tools Bash`; that README also records what the no-plugin baseline showed.
