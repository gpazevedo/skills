...
- **[requirement-traceability](./skills/engineering/requirement-traceability/SKILL.md)**: Opt-in convention that ties each requirement in a spec to its tests. Run it after `/to-tickets` to add requirement IDs, a seam table and a `Covers:` line per ticket; `tdd` then tags tests with the IDs and `implement` runs a coverage gap check before review. A spec without IDs changes nothing.
