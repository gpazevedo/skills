...
- **[requirement-traceability](./requirement-traceability/SKILL.md)**: Opt-in convention that ties each requirement in a spec to a test: requirement IDs on the user stories, a seam table, ID-prefixed test names, a `Covers:` line per issue, and a coverage gap check. A spec without IDs changes nothing. Model-invoked.
