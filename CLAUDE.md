...
Run `scripts/test-traceability-check.sh` after touching the coverage gap check or the `Covers:` structural check in [requirement-traceability](./skills/engineering/requirement-traceability/SKILL.md). It extracts those bash blocks from the `SKILL.md` and runs it against `scripts/fixtures/traceability`, so each block is tested as the skill actually writes it. Behaviour checks on what the model does with a skill live in [evals/](./evals/README.md), run with `claude plugin eval . --scaffold --allow-tools Bash`; that README also records what the no-plugin baseline showed.
